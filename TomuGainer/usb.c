...
// for Gainer
extern char gainer_command_buffer[];
extern int gainer_cmd_place;
extern bool gainer_analog_cont_mode;
extern bool gainer_led;
extern bool gainer_debug;
...
bool gainer_command(char target_cmd, char input_cmd, int place) {
  return(place>=2 && (input_cmd == target_cmd));
}
...
static void cdcacm_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
{
    ...
    if (len) {
      buf[len]=0;
      if (buf[0]=='*' || (buf[0]=='Q' && buf[1]=='*')) {
	usb_puts("Q*");
      } else if(buf[0]=='?' && buf[1]=='*') {
	usb_puts("?1.0.0.00*");
      } else if(buf[0]=='i' && buf[1]=='*'){
	gainer_analog_cont_mode = true;
	usb_puts("i*");
      } else if(buf[0]=='E' && buf[1]=='*'){
	gainer_analog_cont_mode = false;
	usb_puts("E*");
	...
	gainer_led = true;
        usb_puts("h*");
      } else if(strcmp("KONFIGURATION_1*",buf)==0){
	usb_puts("KONFIGURATION_1*");
	/*
      } else if(strcmp("V0*",buf)==0){
        usb_puts("V0*");
	*/
      }
    }
...
void usb_gets(const char *s)
{
    char buf[64];
    int len = usbd_ep_read_packet(g_usbd_dev, 0x01, buf, sizeof(buf));
    ...
    if (len) {
        ...
        usbd_ep_write_packet(g_usbd_dev, 0x82, buf, len);
        buf[len] = 0;
    }
}
